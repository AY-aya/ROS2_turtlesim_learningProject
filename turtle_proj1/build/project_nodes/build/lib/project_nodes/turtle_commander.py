...
class TurtleCommander(Node):
    ...
    def draw_butterfly_variant(self, step=0.1, t_max=12*math.pi):
        cx, cy = 5.5, 5.5
        t = 0.0
        while t < t_max and self.is_drawing:
            exp_term = math.e**(math.cos(t)) - 2*math.cos(4*t)
            x = math.sin(t) * exp_term
            y = math.cos(t) * exp_term
            self.move_to(cx + 0.4*x, cy + 0.4*y)
            t += step
