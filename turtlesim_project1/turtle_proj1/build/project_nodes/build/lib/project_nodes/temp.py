...
class TurtleCommander(Node):
    ...
    def shape_reciever(self, msg:String):
        ...
        if self.shape == "clear":
            ...
            self.is_drawing = False
            self.stop_turtle()
        elif not self.is_drawing:  # Only start new shape if not already drawing
            self.is_drawing = True
            # Use a timer to start drawing to avoid blocking the callback
            self.create_timer(0.05, self.start_drawing)
    
    def start_drawing(self):
        """Start drawing the shape (called by timer to avoid blocking callback)"""
        self.destroy_timer(self._timers[-1])  # Remove this timer
    ...
    # ---------- Shape drawing methods (copied from working code) ----------
    ...
    def draw_infinity(self, a=2.0, step=0.05):
    ...
    def draw_flower(self, petals=6, radius=2.0, step=0.05):
        """Draw flower shape using rose curve"""
        if not self.is_drawing: return
